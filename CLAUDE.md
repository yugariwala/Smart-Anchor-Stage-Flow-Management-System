...
| M0b | `apps/web` scaffold: anonymous sign-in + connectivity probe | **done**, pending a live browser check — needs `VITE_FIREBASE_*` in `apps/web/.env.local` |
...
| M4 | Organizer console and anchor view; Gemini script proposals, structured validation, quota lease, template fallback | not started — needs `wrangler secret put GEMINI_API_KEY` |
...
**The backend is demo-complete for the §25 video** except announcements (M5) and approved
script copy (M4). See `docs/decisions.md` for what that means precisely.
